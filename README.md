...
- **Chất lượng ảnh** — chọn ngay trên web UI (radio) hoặc CLI `--quality`:
  - `balanced` (mặc định): WebP q92 + ảnh nhỏ lossless → nhẹ (~85-90% < PNG), đủ nét.
  - `high`: WebP q97 + nhiều ảnh lossless hơn → nét hơn, nặng hơn chút.
  - `png`: **ảnh gốc PNG** (nét tối đa, nặng nhất).
  - Tinh chỉnh sâu qua `.env`/env: `PSD2HTML_ASSET_FMT`, `PSD2HTML_WEBP_QUALITY`, `PSD2HTML_WEBP_LOSSLESS_MAX` (tham số truyền vào ưu tiên hơn env).
...
- **content-visibility** (slices + React/Next): mỗi section là 1 lớp riêng, trình duyệt bỏ qua render section ngoài màn hình → cuộn mượt hơn.

**Tương tác (chuẩn prod) — tự động, cả slices lẫn React/Next:**
- **Hover**: nút/menu phóng nhẹ + sáng + glow, con trỏ tay, chuyển mượt.
- **Nav cuộn + scroll-spy**: bấm mục menu → cuộn mượt tới section; mục đang xem tự sáng (active). Dùng `getBoundingClientRect` nên khớp cả khi trang bị `transform:scale`.
- **Nút mở link thật**: điền URL vào file `landing.config.js` (React/Next) hoặc object `LINKS` trong `<script>` (slices) theo loại (download/login/topup/gift/...). Để trống → nút hiện popup mẫu (chỗ cắm API/điều hướng thật).
- **Popup/modal**: nút chưa có link mở popup thông báo (đăng nhập/nhận quà/...); đóng bằng nút, click nền, hoặc Esc.
- **Scroll-reveal**: section (trừ section đầu) fade-up khi vào màn hình.

**Tuỳ chọn bám prod (React/Next) — panel trên web UI / cờ CLI:**
- `--swiper-lib`: dùng **thư viện Swiper.js thật** (`effect: fade`, mousewheel tinh chỉnh như prod) thay cho slider tự viết — tự thêm `swiper` vào package.json.
- `--env-config`: link/API đọc từ **`.env`** (`VITE_APP_LINK_*` cho Vite/React, `NEXT_PUBLIC_LINK_*` cho Next) + sinh sẵn file `.env`.
- **VPlay/OIDC login** (checkbox Web UI, mac dinh TAT): sinh helper `handleLogin`, state ngau nhien bang Web Crypto va cac bien `VITE_APP_*` / `NEXT_PUBLIC_APP_*` trong `.env`; action `login` tu dong dung flow nay.
- `--nav-menu`: nav dạng **chữ, config được** (`NAV` array: sửa tên mục/thứ tự), thay nav ảnh; bấm → `slideTo`/scroll, tự active.
- `--popups`: sinh **hệ popup** (login / thể lệ / lịch sử / nạp đầu... stub) — nút CTA bấm mở popup tương ứng, sẵn chỗ cắm nội dung/API.
- `--ai-enhance` ✨ (cần `ANTHROPIC_API_KEY` trong `.env`): AI **"prod-hoá" từng section** — chữ thường (menu/số/mô tả) → **text thật** (`<h2>/<p>`, nét retina + SEO), chữ cách điệu/nghệ thuật giữ ảnh, nút CTA → `<button>` hover; giữ pixel-perfect. Mỗi section 1 lần gọi AI (song song). Key đặt 1 lần trong `.env` (đã gitignore), tool tự đọc.

**Chế độ full-page (swiper) — cờ `--swiper` / checkbox web UI:** thay vì cuộn trôi liên tục, mỗi section chiếm nguyên 1 màn; lăn chuột / vuốt / phím mũi tên / bấm nav → **chuyển section bằng hiệu ứng crossfade (fade)** — section cũ mờ đi, section mới hiện lên tại chỗ, rất mượt (giống Swiper `effect:'fade'`). Không cần thư viện ngoài (vanilla JS / React refs). Áp cho cả slices lẫn React/Next. Mặc định TẮT (cuộn thường).
