...
- **WebP**: asset xuất `.webp` → nhẹ hơn ~85-90% so PNG, tải + giải mã nhanh, đỡ giật. Ảnh lớn (nền/nhân vật) dùng **lossy quality 92** (nét); ảnh nhỏ (chữ/icon/logo) dùng **lossless** (sắc nét tuyệt đối). Chỉnh: `PSD2HTML_WEBP_QUALITY=95` (nét hơn), `PSD2HTML_WEBP_LOSSLESS_MAX=300000` (ngưỡng diện tích px để lossless), `PSD2HTML_ASSET_FMT=png` (quay lại PNG).
