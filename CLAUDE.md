...
"machine" part — read the PSD accurately (layers, coordinates, text, color, export assets) — and Claude
...
Preview a generated `output/` folder (see `.claude/launch.json`):
```powershell
...
- **`ai_enhance.py`** — optional `--ai-enhance` pass: for each generated React/Next section, asks Claude to
...
- **`ai_convert.py`** — Phase 2 for the AI HTML modes (`--one-shot` / `--sections`, i.e. *not*
  slices/react/next): sends the screenshot + layout to Claude and gets back full HTML/CSS. Pages taller
